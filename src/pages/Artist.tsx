...
export default function Artist() {
  ...
  const heroImage = data?.thumbnails?.banner || avatar || null;
  ...
  return (
    <div className="min-h-screen bg-neutral-950 pb-24 text-white">
      <div className="relative mx-auto max-w-5xl">
        <div className="relative h-[280px] w-full overflow-hidden rounded-b-3xl bg-gradient-to-b from-neutral-800 to-neutral-950 sm:h-[320px] md:h-[360px]">
          {heroImage ? <img src={heroImage} alt={artistName} className="absolute inset-0 h-full w-full object-cover" /> : null}
          <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-black/35 to-neutral-950" />
          <div className="absolute left-4 top-4 z-10">
            <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="backdrop-blur bg-black/30 text-white hover:bg-black/50">
              <ArrowLeft className="h-5 w-5" />
            </Button>
          </div>
          <div className="absolute bottom-8 left-0 w-full px-4 sm:px-6">
            <div className="flex items-end gap-4">
              <div className="h-[72px] w-[72px] overflow-hidden rounded-full border border-white/15 bg-neutral-800/80 shadow-lg md:h-24 md:w-24">
        ...
        <div className="px-4 pt-6 sm:px-6">
          {error ? <div className="mt-2 rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-200">{error}</div> : null}

          {loading ? (
            <div className="mt-6 text-sm text-neutral-400">Loading artist...</div>
          ) : (

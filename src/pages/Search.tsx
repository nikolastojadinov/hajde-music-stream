...
const Search = () => {
  ...
  const showAllContent = debouncedSearch.length === 0;
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-32">
      <div className="p-4 md:p-8">
          ...
          {isLoading ? (
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              {searchResults.tracks.length > 0 && (
                <section>
                  <h2 className="text-2xl font-bold mb-4">
                    {showAllContent ? "Sve pesme" : `Pesme (${searchResults.tracks.length})`}
                  </h2>
              ...
              {searchResults.playlists.length > 0 && (
                <section>
                  <h2 className="text-2xl font-bold mb-4">
                    {showAllContent ? "Sve plejliste" : `Plejliste (${searchResults.playlists.length})`}
                  </h2>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        ...
        {/* Categories - removed, now showing all content instead */}
      </div>
    </div>
  );
};

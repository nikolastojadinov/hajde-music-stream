...
export default function MostPopularSection({ snapshot, loading, error, onRetry }: Props) {
  const normalizedItems = (snapshot?.items || []).map(adaptMostPopularSnapshotItem).filter(Boolean);
  ...
  const handleClick = (browseId: string) => {
    trackActivityClient({
      ...
      entityId: browseId,
      context: { section: "most-popular" },
      clientLogMessage: `[Activity] home click playlist id=${browseId} section=most-popular`,
    });
  };
